...
def WideResNet28_10(weights='cifar100'):
    return WideResidualNetwork(depth=28, width=10, dropout_rate=0.3,
                               include_top=True, weights=weights,
                               input_tensor=None, input_shape=None,
                               classes=100, activation='softmax', model_name='wideresnet28-10')


def WideResidualNetwork(depth=28, width=8, dropout_rate=0.0,
                        include_top=True, weights='cifar10',
                        input_tensor=None, input_shape=None,
                        classes=10, activation='softmax', model_name='wide-resnet'):
...
def __conv1_block(input):
    x = Conv2D(16, (3, 3), padding='same', kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay))(input)

    channel_axis = 1 if K.image_data_format() == 'channels_first' else -1

    x = BatchNormalization(axis=channel_axis, beta_regularizer=l2(weight_decay), gamma_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
    return x
...
def __conv2_block(input, k=1, dropout=0.0):
    ...
    if K.image_data_format() == 'channels_first':
        if init._keras_shape[1] != 16 * k:
            init = Conv2D(16 * k, (1, 1), activation='linear', padding='same', kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay))(init)
    else:
        if init._keras_shape[-1] != 16 * k:
            init = Conv2D(16 * k, (1, 1), activation='linear', padding='same', kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay))(init)

    x = Conv2D(16 * k, (3, 3), padding='same', kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay), kernel_initializer='he_normal')(input)
    x = BatchNormalization(axis=channel_axis, beta_regularizer=l2(weight_decay), gamma_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
    ...
    x = Conv2D(16 * k, (3, 3), padding='same', kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay), kernel_initializer='he_normal')(x)
    x = BatchNormalization(axis=channel_axis, beta_regularizer=l2(weight_decay), gamma_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
...
def __conv3_block(input, k=1, dropout=0.0, stride=1):
    init = input
    ...
    if K.image_data_format() == 'channels_first':
        if init._keras_shape[1] != 32 * k:
            init = Conv2D(32 * k, (1, 1), activation='linear', padding='same', kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay))(init)
    else:
        if init._keras_shape[-1] != 32 * k:
            init = Conv2D(32 * k, (1, 1), activation='linear', padding='same', kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay))(init)

    x = Conv2D(32 * k, (3, 3), padding='same', kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay), strides=stride, kernel_initializer='he_normal')(input)
    x = BatchNormalization(axis=channel_axis, beta_regularizer=l2(weight_decay), gamma_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
    ...
    x = Conv2D(32 * k, (3, 3), padding='same', kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay), kernel_initializer='he_normal')(x)
    x = BatchNormalization(axis=channel_axis, beta_regularizer=l2(weight_decay), gamma_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
...
def ___conv4_block(input, k=1, dropout=0.0, stride=1):
    init = input
    ...
    if K.image_dim_ordering() == 'th':
        if init._keras_shape[1] != 64 * k:
            init = Conv2D(64 * k, (1, 1), activation='linear', padding='same', kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay))(init)
    else:
        if init._keras_shape[-1] != 64 * k:
            init = Conv2D(64 * k, (1, 1), activation='linear', padding='same', kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay))(init)

    x = Conv2D(64 * k, (3, 3), padding='same', kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay), strides=stride, kernel_initializer='he_normal')(input)
    x = BatchNormalization(axis=channel_axis, beta_regularizer=l2(weight_decay), gamma_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
    ...
    x = Conv2D(64 * k, (3, 3), padding='same', kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay), kernel_initializer='he_normal')(x)
    x = BatchNormalization(axis=channel_axis, beta_regularizer=l2(weight_decay), gamma_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
...
def __create_wide_residual_network(nb_classes, img_input, include_top, depth=28,
    ...
    for i in range(N):
        x = __conv3_block(x, width, dropout, 2 if i == 0 else 1)
        nb_conv += 2

    x = MaxPooling2D((2, 2))(x)

    for i in range(N):
        x = ___conv4_block(x, width, dropout, 2 if i == 0 else 1)
        nb_conv += 2
