...
def expand_conv(init, base, k, strides=(1, 1)):
    x = Conv2D(base * k, (3, 3), padding='same', strides=strides, kernel_initializer='he_normal',
               kernel_regularizer=l2(weight_decay),
               bias_regularizer=l2(weight_decay))(init)
...
def conv1_block(input, k=1, dropout=0.0):
    ...
    x = BatchNormalization(axis=channel_axis, momentum=0.1, epsilon=1e-5, gamma_initializer='uniform',
                           beta_regularizer=l2(weight_decay), gamma_regularizer=l2(weight_decay))(input)
    x = Activation('relu')(x)
...
def create_wide_residual_network(input_dim, nb_classes=100, N=2, k=1, dropout=0.0, verbose=1):
    ...
    x = Dense(nb_classes, kernel_regularizer=l2(weight_decay), bias_regularizer=l2(weight_decay), activation='softmax')(x)
