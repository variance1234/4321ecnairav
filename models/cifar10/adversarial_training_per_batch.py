...
class DataSubset(object):
    ...
    def get_next_batch(self, batch_size, multiple_passes=False, reshuffle_after_pass=True):
        ...
        if not multiple_passes:
            ...
            batch_xs = self.aug_x[self.cur_order[self.batch_start : batch_end], ...]
    ...
    def augmentation(self):
        ...
        self.aug_x = np.pad(self.xs, ((0, 0), (2, 2), (2, 2), (0, 0)), 'constant', constant_values=(0, 0))
        w, h = self.xs.shape[1], self.xs.shape[2]
        rangew = (w - random_crop_size[0])
        rangeh = (h - random_crop_size[1])
        offsetw = 0 if rangew == 0 else np.random.randint(rangew)
        offseth = 0 if rangeh == 0 else np.random.randint(rangeh)
        self.aug_x = self.aug_x[:, offsetw:offsetw + random_crop_size[0], offseth:offseth + random_crop_size[1]]

        # mean = np.mean(self.aug_x, axis=(1, 2, 3))
        # std = np.std(self.aug_x, axis=(1, 2, 3))
        # N = np.prod(self.xs.shape[1:3])
        #
        # adjust_std = np.maximum(std, 1.0/np.sqrt(N))
        # print(mean.shape)
        # self.aug_x = (self.aug_x - mean.reshape([len(mean), 1, 1, 1]))/adjust_std.reshape([len(adjust_std), 1, 1, 1])


def per_image_standardization(images):
    mean = np.mean(images, axis=(1, 2, 3))
    std = np.std(images, axis=(1, 2, 3))
    N = np.prod(images.shape[1:3])

    adjust_std = np.maximum(std, 1.0 / np.sqrt(N))
    standardized_images = (images - mean.reshape([len(mean), 1, 1, 1])) / adjust_std.reshape([len(adjust_std), 1, 1, 1])
...
def lr_schedule(step):
    if step > 60000:
        ...
        raise Exception("Step can't be negative!")
    print('Learning rate: ', lr)
    return lr


def train(model_path, version, n, attack='fgsm'):
    # Training parameters
    max_steps = 80000
    ...
    model = resnet_models_adv.resnet_v1(input_shape=input_shape, depth=depth, weight_decay=weight_decay, num_classes=10)
    ...
    x_input = K.placeholder(shape=(None, 32, 32, 3), dtype='float32')
    pad = K.spatial_2d_padding(x_input, padding=((2, 2), (2, 2)))
    ...
    for ii in range(max_steps):
        ...
        K.set_value(model.optimizer.lr, lr_schedule(ii))
        ...
        shuffle_index = np.array(range(len(x_batch)))
        np.random.shuffle(shuffle_index)
        rate_of_normal = 0.7
        normal_input = int(len(shuffle_index)*rate_of_normal)
        print(normal_input)
        shuffle_index = shuffle_index[:normal_input]
        print(shuffle_index)
        x_batch_adv = np.array(x_batch_adv)
        x_batch_adv[shuffle_index] = x_batch[shuffle_index]
        print(np.linalg.norm(np.abs(x_batch_adv - x_batch).flatten(), ord=np.inf))
...
def main():
    ...
    # train(model_path='./ResNet32v1_pgd.h5', version=1, n=5, attack='pgd')
    train(model_path='./ResNet32v1_fgsm.h5', version=1, n=5, attack='fgsm')
...
if __name__ == "__main__":
    if (K.backend() == 'tensorflow'):
        ...
        K_random_seed = 451760341
        np_random_seed = 216105420

        tf.set_random_seed(K_random_seed)
