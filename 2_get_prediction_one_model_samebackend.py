...
def main():
    ...
    try:
        ...
        exception_filename = 'predict_exception_%s_%s_%s_%s_%s_%s_%d_%s.txt' % (
        ...
        for stopping_type in ['best_acc', 'best_loss']:
            output_filename = stopping_type + '_output_%s_%s_%s_%s_%s_%s_%d_%s_%d.h5' % (
                args.keras_version, args.backend, args.backend_version, args.cuda_version, args.cudnn_version,
                model_name, random_seed, training_type, iTry)
            weight_filename = stopping_type + '_weight_%s_%s_%s_%s_%s_%s_%d_%s_%d.h5' % (
                args.keras_version, args.backend, args.backend_version, args.cuda_version, args.cudnn_version,
                model_name, random_seed, training_type, iTry)
            perfomance_filename = stopping_type + '_predict_performance_%s_%s_%s_%s_%s_%s_%d_%s_%d.csv' % (
                args.keras_version, args.backend, args.backend_version, args.cuda_version, args.cudnn_version,
                model_name, random_seed, training_type, iTry)

            w_path = args.result_dir + '/' + dataset + '/' + weight_filename
            # check if weight file is there, otherwise do not run
            w_file = Path(w_path)
            ...
            # Load the model
            model = models_list.get_property(model_name, models_list.INIT_METHOD)(load_weight=False)
        ...
        except_filename = 'predict_exception_%s_%s_%s_%s_%s_%s_%d_%s.txt' % (
            args.keras_version, args.backend, args.backend_version, args.cuda_version, args.cudnn_version, model_name,
